from tkinter import *
from PIL import ImageTk, Image
import random

root = Tk()
root.title("JuegoCartasPokemonPorqueNoEncontréCartasDeUno")
root.geometry("800x600")
root.configure(background="saddle brown")

abra = ImageTk.PhotoImage(Image.open ("abra.jpg"))
bulbasour = ImageTk.PhotoImage(Image.open ("bulbasour.jpg"))
charmender = ImageTk.PhotoImage(Image.open ("charmender.jpg"))
Iyvasour = ImageTk.PhotoImage(Image.open ("Iyvasour.jpg"))
jigglypuff = ImageTk.PhotoImage(Image.open ("jigglypuff.jpg"))
kadabra = ImageTk.PhotoImage(Image.open ("kadabra.jpg"))
meowth = ImageTk.PhotoImage(Image.open ("meowth.jpg"))
nidoking = ImageTk.PhotoImage(Image.open ("nidoking.jpg"))
paras = ImageTk.PhotoImage(Image.open ("paras.jpg"))
persion = ImageTk.PhotoImage(Image.open ("persion.jpg"))
pikachu = ImageTk.PhotoImage(Image.open ("pikachu.jpg"))
ratata = ImageTk.PhotoImage(Image.open ("ratata.jpg"))
squirtle = ImageTk.PhotoImage(Image.open ("squirtle.jpg"))
boton = ImageTk.PhotoImage(Image.open ("boton.png"))

player1 = Label(root, text="Player 1", bg="dodger blue", fg="white")
player1.place(relx=0.1,rely=0.3,anchor= CENTER)

player2 = Label(root, text="Player 2", bg="red2", fg="white")
player2.place(relx=0.9,rely=0.3,anchor= CENTER)

player1_score_label = Label(root, bg="dodger blue", fg="white")
player1_score_label.place(relx=0.1,rely=0.4,anchor= CENTER)

player2_score_label = Label(root, bg="red2", fg="white")
player2_score_label.place(relx=0.9,rely=0.4,anchor= CENTER)

pokemon_list = [abra,bulbasour,charmender,Iyvasour,jigglypuff,kadabra,meowth,nidoking,paras,persion,pikachu,ratata,squirtle]
power_list = [200,60,100,50,50,40,70,60,70,30,70]

label = Label(root, bg="tan1")
label.place(relx=0.5,rely=0.5,anchor=CENTER)

player1_score = 0
def player1():
    global player1_score
    random_no = random.randint(0,12)
    random_pokemon = pokemon_list[random_no]
    label["image"] = random_pokemon
    
    random_power_list = power_list[random_no]
    player1_score = player1_score + random_power_list
    player1_score_label["text"] = str(player1_score)
    
player1_btn = Button(root, image = boton,command=player1)
player1_btn.place(relx=0.1,rely=0.6,anchor=CENTER)


player2_score = 0
def player2():
    global player2_score
    random_no1 = random.randint(0,12)
    random_pokemon1 = pokemon_list[random_no1]
    label["image"] = random_pokemon1
    
    random_power_list1 = power_list[random_no1]
    player2_score = player2_score + random_power_list1
    player2_score_label["text"] = str(player2_score)
    
player2_btn = Button(root, image = boton,command=player2)
player2_btn.place(relx=0.9,rely=0.6,anchor=CENTER)


root.mainloop()